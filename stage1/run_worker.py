...
print("[stage1 worker] process started; loading PyTorch (first import can take 10–60s)…", flush=True)

import torch.distributed.rpc as rpc
from torch.distributed.rpc import TensorPipeRpcBackendOptions

# Ensure RPC can resolve worker_ops.* on this process
import stage1.worker_ops  # noqa: F401

print("[stage1 worker] imports done.", flush=True)
...
def main() -> None:
    ...
    print(
        f"[stage1 worker] rendezvous MASTER_ADDR={args.master_addr!r} MASTER_PORT={args.master_port}",
        flush=True,
    )
    if args.master_addr in ("127.0.0.1", "localhost"):
        print(
            "[stage1 worker] NOTE: driver in Docker must use the host LAN IP for MASTER_ADDR, "
    ...
    print(
        "[stage1 worker] calling rpc.init_rpc — this BLOCKS here until the driver (rank=0) "
    ...
    print(
        f"[stage1 worker] name={args.name!r} rank={args.rank} world_size={args.world_size} "
