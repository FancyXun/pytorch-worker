# Baseline RPC remote training (`rpc_remote_training`)

The `rpc_remote_training` package implements a product-style training split using **official PyTorch RPC**:
...
This baseline keeps model compute on a trusted GPU worker and keeps user-side code mostly standard PyTorch.
...
- Worker entry: `rpc_remote_training.run_worker`
- Driver API: `rpc_remote_training.driver_api.Stage1Trainer`
- Core worker ops: `rpc_remote_training.worker_ops`
...
4. Start from the directory that contains the `rpc_remote_training/` folder (important for `python -m rpc_remote_training.xxx`).
...
docker run -itd \
  --name rpc-remote-training-driver \
...
```bash
docker exec -it rpc-remote-training-driver bash
...
PYTHONUNBUFFERED=1 python -m rpc_remote_training.run_worker
```
...
export MASTER_ADDR=10.60.82.27
export MASTER_PORT=29500
PYTHONUNBUFFERED=1 python -m rpc_remote_training.example_train
```

Or two-input model:

```bash
python -m rpc_remote_training.example_two_input
```
...
1. **`ModuleNotFoundError: No module named 'rpc_remote_training'`**
   - Run from project root (the parent directory containing `rpc_remote_training/`), not inside `rpc_remote_training/`.
...
```bash
docker rm -f rpc-remote-training-driver
```
...
- [Extended RPC (`rpc_remote_training_v2`)](../rpc_remote_training_v2/README.md)
