...
def compute_metrics(true_df, pred_df, metric='rmse'):
  ...
  if metric not in ('rmse', 'mae', 'mape'):
    raise ValueError("metric must be one of: 'rmse', 'mae', 'mape'")
  ...
  for i in range(true_df.shape[1]):
    ...
    y_pred = pred_df[:, i]
    if metric == 'rmse':
      m = np.mean(np.square(y_true - y_pred))
    elif metric == 'mae':
      m = np.mean(np.abs(y_true - y_pred))
    else:
      denom = np.maximum(np.abs(y_true), 1e-8)
      m = np.mean(np.abs((y_true - y_pred) / denom))
    output.append(m)
