...
# Example:
#   source /path/to/auto_ddp/setup_env_trainer.sh && python3 train.py
...
echo "[auto-ddp env trainer] master=${MASTER_ADDR}:${MASTER_PORT} rank=${RANK} trainer_rank=${TORCH_DDP_TRAINER_RANK} world_size=${WORLD_SIZE} iface=${GLOO_SOCKET_IFNAME}"
